...
那是因为你不子啊这个分支上，你需要切换回到这个 `master` 分支，然后在进行操作：
...
```javascript
git diff branchOne branchTwo -- path/fileName.txt
```
